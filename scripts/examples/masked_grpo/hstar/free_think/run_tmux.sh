...
tmux send-keys -t "$SERVER_SESSION" "source /root/miniconda3/etc/profile.d/conda.sh && conda activate vagen" C-m
...
tmux send-keys -t "$TRAIN_SESSION" "source /root/miniconda3/etc/profile.d/conda.sh && conda activate vagen" C-m
